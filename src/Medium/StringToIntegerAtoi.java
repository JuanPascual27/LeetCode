...
/*
    ...
    4. Rounding: If the integer is out of the 32-bit signed integer range [-2^(31), 2^(31) - 1], then round the
        integer to remain in the range. Specifically, integers less than -2^(31) should be rounded to -2^(31),
...
public class StringToIntegerAtoi {
    public int myAtoi(String s) {
        ...
        else if(s.charAt(i) == '+') i++; //Step 2
        while(i < s.length()) {
            char c = s.charAt(i);
            if(c < '0' || c > '9') break; //Step 3
            if (!isNeg && res > (Integer.MAX_VALUE - (c - '0'))/10) return Integer.MAX_VALUE;
            if (isNeg && -res < (Integer.MIN_VALUE + (c - '0'))/10) return Integer.MIN_VALUE; //Step 4
            res = res * 10 + (c - '0');
            i++;
        }
